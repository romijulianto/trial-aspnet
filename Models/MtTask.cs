...
public partial class trial_05082023 {
    ...
    /// </summary>
    [MaybeNull]
    public static MtTask mtTask
    {
        get => HttpData.Resolve<MtTask>("mt_task");
        set => HttpData["mt_task"] = value;
    }

    /// <summary>
    /// Table class for mt_task
    /// </summary>
    public class MtTask : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<NpgsqlDbType> id;

        public readonly DbField<NpgsqlDbType> _name;

        public readonly DbField<NpgsqlDbType> deadline;

        public readonly DbField<NpgsqlDbType> createat;

        public readonly DbField<NpgsqlDbType> updateat;
        ...
        public MtTask()
        {
            ...
            Name = "mt_task";
            Type = "TABLE";
            UpdateTable = "mt_task"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            // id
            id = new (this, "x_id", 3, NpgsqlDbType.Integer) {
                Name = "id",
                Expression = "\"id\"",
                BasicSearchExpression = "CAST(\"id\" AS varchar(255))",
                DateTimeFormat = -1,
                ...
                CustomMessage = Language.FieldPhrase("mt_task", "id", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("id", id);

            // name
            _name = new (this, "x__name", 200, NpgsqlDbType.Varchar) {
                Name = "name",
                Expression = "\"name\"",
                UseBasicSearch = true,
                ...
                CustomMessage = Language.FieldPhrase("mt_task", "_name", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("name", _name);

            // deadline
            deadline = new (this, "x_deadline", 133, NpgsqlDbType.Date) {
                Name = "deadline",
                Expression = "\"deadline\"",
                BasicSearchExpression = CastDateFieldForLike("\"deadline\"", 0, "DB"),
                DateTimeFormat = 0,
                VirtualExpression = "\"deadline\"",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("mt_task", "deadline", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("deadline", deadline);

            // createat
            createat = new (this, "x_createat", 133, NpgsqlDbType.Date) {
                Name = "createat",
                Expression = "\"createat\"",
                BasicSearchExpression = CastDateFieldForLike("\"createat\"", 0, "DB"),
                DateTimeFormat = 0,
                VirtualExpression = "\"createat\"",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                DefaultErrorMessage = ConvertToString(Language.Phrase("IncorrectDate")).Replace("%s", CurrentDateTimeFormat.ShortDatePattern),
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("mt_task", "createat", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("createat", createat);

            // updateat
            updateat = new (this, "x_updateat", 133, NpgsqlDbType.Date) {
                Name = "updateat",
                Expression = "\"updateat\"",
                BasicSearchExpression = CastDateFieldForLike("\"updateat\"", 0, "DB"),
                DateTimeFormat = 0,
                VirtualExpression = "\"updateat\"",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                DefaultErrorMessage = ConvertToString(Language.Phrase("IncorrectDate")).Replace("%s", CurrentDateTimeFormat.ShortDatePattern),
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("mt_task", "updateat", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("updateat", updateat);
        ...
        // Connection
        public virtual DatabaseConnectionBase<NpgsqlConnection, NpgsqlCommand, NpgsqlDataReader, NpgsqlDbType> Connection => GetConnection(DbId);
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                deadline.DbValue = row["deadline"]; // Set DB value only
                createat.DbValue = row["createat"]; // Set DB value only
                updateat.DbValue = row["updateat"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "mttaskview"))
                return Language.Phrase("View");
            else if (SameString(pageName, "mttaskedit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "mttaskadd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "MtTaskView",
                Config.ApiAddAction => "MtTaskAdd",
                Config.ApiEditAction => "MtTaskEdit",
                Config.ApiDeleteAction => "MtTaskDelete",
                Config.ApiListAction => "MtTaskList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "mttasklist";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("mttaskview", parm);
            else
                url = KeyUrl("mttaskview", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "mttaskadd";
        ...
        {
            string url = "";
            url = KeyUrl("mttaskedit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("mttasklist", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("mttaskadd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("mttasklist", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("mttaskdelete")); // DN
        ...
        public async Task<DbDataReader?> LoadReader(string filter, string sort = "", DatabaseConnectionBase<NpgsqlConnection, NpgsqlCommand, NpgsqlDataReader, NpgsqlDbType>? conn = null)
        {
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            ...
            deadline.SetDbValue(dr["deadline"]);
            createat.SetDbValue(dr["createat"]);
            updateat.SetDbValue(dr["updateat"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "MtTaskList";
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    ...
                    if (exportType == "view") {
                        ...
                        doc.ExportCaption(deadline);
                        doc.ExportCaption(createat);
                        doc.ExportCaption(updateat);
                    } else {
                        doc.ExportCaption(id);
                        doc.ExportCaption(_name);
                        doc.ExportCaption(deadline);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(deadline);
                            await doc.ExportField(createat);
                            await doc.ExportField(updateat);
                        } else {
                            await doc.ExportField(id);
                            await doc.ExportField(_name);
                            await doc.ExportField(deadline);
