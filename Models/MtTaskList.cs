...
public partial class trial_05082023 {
    ...
    /// mtTaskList
    /// </summary>
    public static MtTaskList mtTaskList
    {
        get => HttpData.Get<MtTaskList>("mtTaskList")!;
        set => HttpData["mtTaskList"] = value;
    }

    /// <summary>
    /// Page class for mt_task
    /// </summary>
    public class MtTaskList : MtTaskListBase
    {
        // Constructor
        public MtTaskList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtTaskList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtTaskListBase : MtTask
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "mt_task";

        // Page object name
        public string PageObjName = "mtTaskList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fmt_tasklist";
        ...
        public MtTaskListBase()
        {
            ...
            // Table object (mtTask)
            if (mtTask == null || mtTask is MtTask)
                mtTask = this;
            ...
            MultiDeleteUrl = "mttaskdelete";
            MultiUpdateUrl = "mttaskupdate";
        ...
        // Page name
        public string PageName => "mttasklist";
        ...
        public void SetVisibility()
        {
            id.SetVisibility();
            _name.SetVisibility();
            deadline.SetVisibility();
            createat.SetVisibility();
            updateat.SetVisibility();
        }

        // Constructor
        public MtTaskListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "mttaskview" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(deadline.AdvancedSearch.ToJson())); // Field deadline
            filters.Merge(JObject.Parse(createat.AdvancedSearch.ToJson())); // Field createat
            filters.Merge(JObject.Parse(updateat.AdvancedSearch.ToJson())); // Field updateat
        ...
        protected bool RestoreFilterList() {
            ...
            // Field deadline
            if (filter?.TryGetValue("x_deadline", out sv) ?? false) {
                deadline.AdvancedSearch.SearchValue = sv;
                deadline.AdvancedSearch.SearchOperator = filter["z_deadline"];
                deadline.AdvancedSearch.SearchCondition = filter["v_deadline"];
                deadline.AdvancedSearch.SearchValue2 = filter["y_deadline"];
                deadline.AdvancedSearch.SearchOperator2 = filter["w_deadline"];
                deadline.AdvancedSearch.Save();
            }

            // Field createat
            if (filter?.TryGetValue("x_createat", out sv) ?? false) {
                createat.AdvancedSearch.SearchValue = sv;
                createat.AdvancedSearch.SearchOperator = filter["z_createat"];
                createat.AdvancedSearch.SearchCondition = filter["v_createat"];
                createat.AdvancedSearch.SearchValue2 = filter["y_createat"];
                createat.AdvancedSearch.SearchOperator2 = filter["w_createat"];
                createat.AdvancedSearch.Save();
            }

            // Field updateat
            if (filter?.TryGetValue("x_updateat", out sv) ?? false) {
                updateat.AdvancedSearch.SearchValue = sv;
                updateat.AdvancedSearch.SearchOperator = filter["z_updateat"];
                updateat.AdvancedSearch.SearchCondition = filter["v_updateat"];
                updateat.AdvancedSearch.SearchValue2 = filter["y_updateat"];
                updateat.AdvancedSearch.SearchOperator2 = filter["w_updateat"];
                updateat.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(deadline); // deadline
                UpdateSort(createat); // createat
                UpdateSort(updateat); // updateat
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""mt_task"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""mt_task"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_tasklist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_tasklist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""mt_task"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("deadline")); // DN
                CreateColumnOption(option.Add("createat")); // DN
                CreateColumnOption(option.Add("updateat")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fmt_tasksrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fmt_tasksrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fmt_tasklist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtTaskList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtTaskList.RowCount) + "_mt_task");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtTaskList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtTaskList.RowType == RowType.Add || IsEdit && mtTaskList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            deadline.SetDbValue(row["deadline"]);
            createat.SetDbValue(row["createat"]);
            updateat.SetDbValue(row["updateat"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("deadline", deadline.DefaultValue ?? DbNullValue); // DN
            row.Add("createat", createat.DefaultValue ?? DbNullValue); // DN
            row.Add("updateat", updateat.DefaultValue ?? DbNullValue); // DN
            return row;
        }

        #pragma warning disable 618, 1998
        // Load old record
        protected async Task<Dictionary<string, object>?> LoadOldRecord(DatabaseConnectionBase<NpgsqlConnection, NpgsqlCommand, NpgsqlDataReader, NpgsqlDbType>? cnn = null) {
            // Load old record
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // deadline
                deadline.ViewValue = deadline.CurrentValue;
                deadline.ViewValue = FormatDateTime(deadline.ViewValue, deadline.FormatPattern);
                deadline.ViewCustomAttributes = "";

                // createat
                createat.ViewValue = createat.CurrentValue;
                createat.ViewValue = FormatDateTime(createat.ViewValue, createat.FormatPattern);
                createat.ViewCustomAttributes = "";

                // updateat
                updateat.ViewValue = updateat.CurrentValue;
                updateat.ViewValue = FormatDateTime(updateat.ViewValue, updateat.FormatPattern);
                updateat.ViewCustomAttributes = "";
                ...
                _name.TooltipValue = "";
                if (!IsExport())
                    _name.ViewValue = _name.HighlightValue(_name.ViewValue);

                // deadline
                deadline.HrefValue = "";
                deadline.TooltipValue = "";

                // createat
                createat.HrefValue = "";
                createat.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fmt_tasksrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            item.Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" + Language.Phrase("Highlight") + "\" data-caption=\"" + Language.Phrase("Highlight") + "\" data-ew-action=\"highlight\" data-form=\"fmt_tasksrch\" data-name=\"" + HighlightName + "\">" + Language.Phrase("HighlightBtn") + "</a>";
